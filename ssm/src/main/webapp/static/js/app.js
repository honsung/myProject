...
var App = function () {
    ...
    var handleInit = function() {
        ...
        if (isIE10) {
            jQuery('html').addClass('ie10'); // 探明了IE10 version
        }
    }

    var handleDesktopTabletContents = function () {
        // loops all page elements with "responsive" class and applies classes for tablet mode  用“响应”类循环所有页面元素，并为平板模式应用类
        // For metornic  1280px or less set as tablet mode to display the content properly  对于metornic 1280px或不设置为平板模式下显示的内容正确
    ...
    var handleSidebarState = function () {
        // remove sidebar toggler if window width smaller than 900(for table and phone mode)  如果窗口宽度小于900移除侧边栏显示开关（表和电话模式）
        if ($(window).width() < 980) {
    ...
    var runResponsiveHandlers = function () {
        // reinitialize other subscribed elements  重新初始化订阅内容等
    ...
    var handleSidebarMenu = function () {
        ...
        jQuery('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
                ...
                $.post(url, {}, function (res) {
                        ...
                        App.fixContentHeight(); // fix content height  固定内容的高度
                        App.initUniform(); // initialize uniform elements 统一的元素初始化
                    });
            });
    }
    ...
    var handleTheme = function () {
        ...
        // handle theme colors 处理主题颜色
        var setColor = function (color) {
            $('#style_color').attr("href", baseDir+"/static/css/" + color + ".css");
    ...
    return {
        ...
        blockUI: function (el, centerY) {
            var el = jQuery(el); 
            el.block({
                    message: '<img src="'+baseDir+'/static/image/ajax-loading.gif" align="">',
